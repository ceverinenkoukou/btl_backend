...
from btl.models import RapportConfig, Campagne
from btl.permissions import IsSuperviseurOrAdmin, IsPasswordChanged
from btl.serializers import RapportConfigSerializer


class RapportConfigViewSet(viewsets.ModelViewSet):
    """
    CRUD configuration rapport d'une campagne.
    ...
    """
    serializer_class = RapportConfigSerializer
    ...
    def get_queryset(self):
        qs = RapportConfig.objects.select_related('campagne', 'configure_par')
    ...
    def par_campagne(self, request):
        """
        GET  /api/rapport-configs/par-campagne/?campagne=<id>
             → retourne la config existante ou les valeurs par défaut (sans créer en base)

        PUT/PATCH /api/rapport-configs/par-campagne/?campagne=<id>
        ...
        if request.method == 'GET':
            try:
                config = RapportConfig.objects.get(campagne=campagne)
                serializer = self.get_serializer(config)
                return Response(serializer.data)
            except RapportConfig.DoesNotExist:
        ...
        config, created = RapportConfig.objects.get_or_create(campagne=campagne)
